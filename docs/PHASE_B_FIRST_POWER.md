...
| B4.4 | **Hall interrupt-rate margin (OD-11):** with the ESC running, log `hallSensor.isrEntries()` and `lastWindowEntries()` over a full-throttle pass, then repeat with the GPIO35 10 kΩ pull-up lifted (scope the pin). Record: entries per 100 ms in normal running (expect ≲ 9), the value with the pull-up lifted, and `guardFaults()`. The guard's bound is 180 entries/100 ms (20× the 5000 rpm maximum), and a window that ran more than 10× long is judged only when it carries ≥ 10× the elapsed-scaled allowance (a 2 s stall: 36 000 entries) — confirm normal running is far below both and that the fault path (detach → rpm 0 → re-arm after 1 s) behaves as the native tests describe. |
