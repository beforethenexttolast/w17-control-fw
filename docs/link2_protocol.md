...
- `pio test -e native` pins the wire format hermetically from this side. **Control-fw only:**
  `test_golden_frame_bytes` fixes the exact 16 bytes, and `test_crc_matches_crsf_implementation`
...
1       1     length = payload byte count (13 in v2)
2       13    payload (below)
15      1     crc8 over bytes [1..14]  (length + payload; start byte excluded)
```
...
v2 appends two payload bytes — `soundProfile` and `volume` — carrying the engine-voice
selection and volume/quiet level from board #1's persisted settings to board #2's synth
(vision decision 15; mechanism decided at the 2026-08-16 owner review). Framing, the CRC
algorithm, field order and every v1 field's offset are unchanged; the length byte goes
11 → 13 and the version byte 1 → 2.
...
```
A5 0D 02 2A E7 4C 03 DC 05 DC 1E 3C 02 01 50 CC
│  │  │  │  │  │  │  └─┴─ rpm 1500    └─┴─ battery 7900  │  │  │  │  └ crc8
│  │  │  │  │  │  └ gear 3                               │  │  │  └ volume 80
│  │  │  │  │  └ flags 0x4C = drsOpen | armed | ersDeploying │  └ soundProfile 1 (V6)
│  │  │  │  └ steeringPercent = 0xE7 = −25                │  └ driveMode 2 (ERS)
│  │  │  └ throttlePercent = 0x2A = +42                   └ ersPercent 60
│  │  └ version 2
│  └ length 13
└ start
```
...
voice V6 turbo-hybrid at volume 80/100.
